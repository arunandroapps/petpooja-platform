...
export default function RestaurantLayout() {
  const { currentUser, logout, restaurants, activeRestaurantId } = usePlatform();
    ...
    <div className="flex h-screen overflow-hidden">
      <aside className="w-60 shrink-0 bg-white border-r border-slate-200 flex flex-col">
        <div className="px-5 py-5 border-b border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-bold text-slate-900 leading-tight text-sm truncate">{rst?.name||'Restaurant'}</div>
              <div className="text-[10px] uppercase tracking-wider text-slate-500">{rst?.type||'Pet Pooja'}</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-200">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-7 h-7 rounded-full bg-brand-500 text-white flex items-center justify-center text-xs font-bold">{currentUser?.name.charAt(0)}</div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-white border-b border-slate-200 px-6 flex items-center justify-between">
          <div className="text-sm text-slate-500">{rst?.city} · {rst?.address?.split(',')[0]}</div>
          <div className="flex items-center gap-3">
            <div className="text-sm font-medium text-brand-700">🍽️ Restaurant</div>
          </div>
        </header>
