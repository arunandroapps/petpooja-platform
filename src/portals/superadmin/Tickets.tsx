...
export default function SATickets() {
  const { tickets, resolveTicket, upsertTicket } = usePlatform();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div key={t.id} className="card p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">{t.fromName} · {t.from} · {fmtDate(t.createdAt)}</div>
                  </div>
                ...
                <div className="flex gap-2">
                  {t.status==='open'&&<button onClick={()=>upsertTicket({...t,status:'in-progress'})} className="btn-secondary text-xs">Mark In-Progress</button>}
                  <button onClick={()=>resolveTicket(t.id)} className="btn-own text-xs"><CheckCircle2 size={12}/>Resolve</button>
                </div>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="space-y-2">
            {resolved.map(t=>(
              <div key={t.id} className="card p-3 opacity-60">
