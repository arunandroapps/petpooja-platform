...
export default function Login() {
  ...
  // Already logged in — redirect declaratively
  if (currentUser) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (u) nav(roleHome[u.role] || '/', { replace: true });
      ...
      <div className="w-full max-w-4xl grid lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="text-white hidden lg:block">
          ...
          <div className="space-y-3">
            {DEMO_CREDS.map(d=>(
              <button key={d.role} onClick={()=>quickLogin(d.email,d.password)}
                className="w-full flex items-center gap-3 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition border border-white/10 text-left">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" className="w-full btn-primary py-2.5">Sign In →</button>
          </form>
          {/* Mobile quick login */}
          <div className="mt-6 lg:hidden">
            ...
            <div className="grid grid-cols-2 gap-2">
              {DEMO_CREDS.map(d=>(
                <button key={d.role} onClick={()=>quickLogin(d.email,d.password)} className={`${d.color} text-white text-xs rounded-lg px-3 py-2 font-medium`}>{d.role}</button>
              ))}
            </div>
          </div>
        </div>
